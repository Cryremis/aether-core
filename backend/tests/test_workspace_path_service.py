...
def build_workspace(root: Path) -> SandboxWorkspace:
    ...
    return SandboxWorkspace(
        workspace_id="ws_paths",
        owner_session_id="sess_paths",
        root=root,
        baseline_root=None,
...
def test_workspace_path_service_normalizes_relative_and_absolute_paths(tmp_path):
    ...
    (workspace.work_dir / "demo.txt").write_text("hello", encoding="utf-8")
    session = AgentSession(session_id="sess_paths", workspace=workspace)

    resolved_relative = workspace_path_service.resolve_path(session, "work/demo.txt")
    resolved_absolute = workspace_path_service.resolve_path(session, "/workspace/work/demo.txt")
    resolved_empty = workspace_path_service.resolve_path(session, None)

    assert resolved_relative.logical_path == "/workspace/work/demo.txt"
    assert resolved_absolute.logical_path == "/workspace/work/demo.txt"
