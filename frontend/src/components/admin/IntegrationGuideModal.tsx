...
export function IntegrationGuideModal({
  ...
  const integrationGuideDetails = [
    {
      title: "推荐接入顺序",
      body: "按顺序复制即可：先复制前端代码，再复制 .env 示例，最后复制后端 Bind 示例。默认代码已经填好平台信息，只需要检查你们平台自己的用户对象和公网地址。",
    },
    {
      title: "前端代码负责什么",
      body: "它负责浮球、抽屉、iframe 和会话 key 持久化这些通用逻辑。大多数平台只需要确认 /static/aethercore-embed.js 能访问，以及 getUserId 能拿到当前登录用户 ID。",
    },
    {
      title: ".env 怎么用",
      body: ".env 示例里已经填好 AetherCore 地址、platform_key 和 platform_secret。你只需要把 AETHERCORE_HOST_CALLBACK_BASE_URL 换成你们平台自己的公网根地址。这个地址会在 bind 时动态注入到 host_callback_base_url，给 AetherCore 回调你们的工具接口使用。如果你们框架不自动加载 .env，也可以把同样的值直接配成环境变量。",
    },
    {
      title: "后端 Bind 示例怎么用",
                ...
                <div className="guide-split-layout__code">
                  <div className="guide-section">
                    ...
                    <div className="guide-section__head">
                      ...
                      <button type="button" className="action-button small primary" onClick={() => onCopy(integrationGuide.snippets.frontend)}>
                        复制
                      </button>
                    </div>
                    <pre className="guide-code-block">
                      <code>{renderHighlightedSnippet(integrationGuide.snippets.frontend)}</code>
                    </pre>
                  </div>
                  ...
                  <div className="guide-section">
                    ...
                    <div className="guide-section__head">
                      ...
                      <button type="button" className="action-button small primary" onClick={() => onCopy(integrationGuide.snippets.backend_env)}>
                        复制
                      </button>
                    </div>
                    <pre className="guide-code-block">
                      <code>{renderHighlightedSnippet(integrationGuide.snippets.backend_env)}</code>
                    </pre>
                  </div>
                  ...
                  <div className="guide-section">
                    ...
                    <div className="guide-section__head">
                      ...
                      <button type="button" className="action-button small primary" onClick={() => onCopy(integrationGuide.snippets.backend_fastapi)}>
                        复制
                      </button>
                    </div>
                    <pre className="guide-code-block">
                      <code>{renderHighlightedSnippet(integrationGuide.snippets.backend_fastapi)}</code>
                    </pre>
                  </div>
                </div>
